...
class Ed25519EdwardsCryptoProvider implements CryptoProvider {
  ...
  @override
  Uint8List digest({required Uint8List data, required DigestAlgorithm algorithm}) {
    throw UnimplementedError();
  }

  @override
  Key generateKeyPair({required KeyParameters keyParameters, Uint8List? seed}) {
    if(keyParameters is EcKeyParameters && keyParameters.curve == Curve.curve25519) {
      if(seed != null) {
        ed.PrivateKey privateKey = ed.newKeyFromSeed(seed);
        ed.PublicKey publicKey = ed.public(privateKey);
        return EdPrivateKey(
          a: Uint8List.fromList(privateKey.bytes),
        ...
        return EdPrivateKey(
            a: Uint8List.fromList(keyPair.privateKey.bytes),
  ...
  @override
  FutureOr<Signature> sign({required Uint8List data, required SigningAlgorithm algorithm}) {
    ed.PrivateKey privateKey = ed.PrivateKey((key as PrivateKey).private);
  ...
  @override
  bool verify({required Uint8List data, required SigningAlgorithm algorithm, required Signature signature}) {
    ed.PublicKey publicKey = ed.PublicKey((key as EdPublicKey).pubA);
    return ed.verify(publicKey, data, Uint8List.fromList((signature as EcSignature).r + signature.s));
  }
